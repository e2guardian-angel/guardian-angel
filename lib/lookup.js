...
Lookup.prototype.lookupHostName = async function(hostname, category) {
    ...
    const categorySql = 'SELECT domains.domainText, categories.categoryText FROM domains ' +
        'INNER JOIN categories ON domains.categoryId = categories.id ' +
        'WHERE domainText = ? AND categoryText = ?;';
    ...
    while(parts.length > 1) {
        ...
        try {
            const generalResult = this.db.get(generalSql, domain)
            const categoryResult = (category === 'any') ? null : await this.db.get(categorySql, domain, category);
            if (categoryResult) {
                return categoryResult;
            } else {
                const result = await generalResult;
                if (!result) {
                    parts = parts.slice(1);
                } else if (category === result.categoryText ||
                    category === 'any') {
                    return result;
                } else {
                    return null;
