...
var Tree = defclass({
    ...
    addHostName: function(hostname, category) {
        ...
        if (parts.length === 0 || parts.indexOf('') > -1) {
    ...
    lookupHostName: function(hostname) {
        let parts = hostname.split('.').reverse();
        if (parts.length === 0) {
            throw Error(`Invalid hostname: ${hostname}`);
        }
        ...
        while (parts.length > 0) {
            ...
            try {
                currentNode = currentNode.getChildById(currentId);
                currentCategory = currentNode.category;
            } catch (err) {
                break;
            }
            if (parts.length > 0) {
        ...
        return {domain: currentNode.id, category: currentCategory};
    }
});
