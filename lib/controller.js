...
const GUARDIAN_CERT_FILE = '/opt/guardian/ssl/public.pem'
const GUARDIAN_KEY_FILE = '/opt/guardian/ssl/private.pem'
...
function loadResources() {
    ...
    resources.configmaps.cert = JSON.parse(fs.readFileSync(GUARDIAN_CERT_RESOURCE_FILE));
    // Load the secrets
...
var Controller = defclass({
    ...
    pushConfig: async function() {
        try {
            ...
            return await this.kubeApply(CONFIGMAPS_PATH, GUARDIAN_CONF_PATH, configResource);
        } catch (err) {
    ...
    // TODO: rotate redis secret
    setGuardianCertificate(certData) {
        // TODO: how to validate certificate data?
        kubeData.cert = certData;
    },
    getGuardianCertificate: function() {
        return kubeData.cert;
    },
    pushGuardianCertificate: async function() {
        try {
            let certResource = duplicateObject(resources.configmaps.cert);
            certResource.data['public.crt'] = certResource;
            return await this.kubeApply(GUARDIAN_CONF_PATH, GUARDIAN_CERT_PATH, certResource);
        } catch(err) {
            console.error(`Failed to certificate: ${err.message}`);
            return null;
        }
    },
    pullGuardianCertificate: async function() {
        try {
            let certResource = await this.kubeGet(GUARDIAN_CERT_PATH);
            kubeData.cert = certResource.body.data['public.crt'];
            return kubeData.cert;
        } catch (err) {
    ...
    getKubeData: async function() {
        try {
            ...
            kubeData.cert = await this.pullGuardianCertificate();
            return kubeData;
        } catch (err) {
