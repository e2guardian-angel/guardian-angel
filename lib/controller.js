...
function Controller() {
    ...
    if (fs.existsSync(NAMESPACE_FILE)) {
        this.namespace = fs.readFileSync(NAMESPACE_FILE);
    } else {
...
Controller.prototype.generatePaths = function(namespace) {
    ...
    this.paths.resources.deployments.nginx = `${this.paths.kube.deployments}/nginx`;
    ...
    this.paths.resources.services.nginx = `${this.paths.kube.services}/nginx`;
};
