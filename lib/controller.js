...
var Controller = defclass({
    ...
    openRedis: async function(kubeData) {
        return await lookup.openRedis(kubeData);
    },
    ...
    renderFilterDeployment: function() {
        let filterResource = duplicateObject(resources.deployments.webfilter);
    ...
    renderFilterService: function() {
        let filterServiceResource = duplicateObject(resources.services.webfilter);
        if (!kubeData.config.transparent) {
