...
var Controller = defclass({
    ...
    deployFilter: async function() {
        ...
        const reload = !savedData.config || !kubeData.config ||
            kubeData.config.localNetwork !== savedData.config.localNetwork ||
            kubeData.config.squidConfigDir !== savedData.config.squidConfigDir ||
            JSON.stringify(kubeData.config.allowRules) !== JSON.stringify(savedData.config.allowRules) ||
            JSON.stringify(kubeData.config.decryptRules) !== JSON.stringify(savedData.config.decryptRules) ||
